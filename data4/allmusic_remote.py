...
def parseCD(cd):
    ...
    for tr in trs: # performance & movement are in the same level
        if tr.attrib["class"] == 'perfomance-title':
            ...
            pInfo['movements'].append(trackInfo)
    #end for
    if bool(pInfo):
        pInfoList.append(pInfo)

    cdTitle = cd.xpath("div/h3")[0].text.strip(" \t\n\r")
    cdTitle = " ".join(cdTitle.split()) ; print(cdTitle) # spaces inside phase
    return OrderedDict([("title", cdTitle), ("performances", pInfoList)])
#end
